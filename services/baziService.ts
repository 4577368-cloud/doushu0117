...
export const interpretAnnualPillar = (chart: BaziChart, annualGz: GanZhi, monthGz?: GanZhi, dayGz?: GanZhi): PillarInterpretation => {
  ...
  Object.entries(pillars).forEach(([name, p]) => {
    ...
    if (isGanClash(annualGan, pGan) && BRANCH_CLASHES[annualZhi] === pZhi) {
      triggers.push(`- **天克地冲·${pName}**：流年与${pName}天克地冲，这是极大的变动信号。${
    ...
    // 4. 六合 (和谐)
      ...
      triggers.push(`- **合·${pName}**：流年与${pName}六合。${
    ...
    if (BRANCH_XING[annualZhi]?.includes(pZhi)) {
         triggers.push(`- **相刑·${pName}**：流年与${pName}形成相刑。${
    ...
    if (BRANCH_HAI[annualZhi] === pZhi) {
        triggers.push(`- **六害·${pName}**：流年与${pName}相害。主犯小人或亲缘冷淡，${
            name === '月' ? '防同僚倾轧，或与兄弟不和' : 
  ...
  if (monthGz && dayGz) {
     ...
     const adviceMap: Record<string, string> = {
          ...
          '七杀': '【七杀日】：魄力增强，利于开拓新局或解决难题。但需防脾气急躁，忌冲动。',
     ...
     // 4. 微观互动 (冲合刑害)
     const interactions: string[] = [];
     if (BRANCH_CLASHES[mZhi] === annualGz.zhi) interactions.push(`【月令冲太岁】（波动）`);
     if (BRANCH_CLASHES[dZhi] === mZhi) interactions.push(`【日月相冲】（情绪易烦躁）`);
     else if (BRANCH_COMBINATIONS[dZhi] === mZhi) interactions.push(`【日月相合】（心情舒畅）`);
