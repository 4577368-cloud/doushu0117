...
import { X, ClipboardCopy, Maximize2, Minimize2 } from 'lucide-react';
...
export const ReportHistoryModal: React.FC<{ report: any; onClose: () => void }> = ({ report, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[2200] flex items-center justify-center p-4">
            ...
            <div className={`relative bg-white shadow-2xl flex flex-col animate-slide-up overflow-hidden ${isFull ? 'w-full h-[95vh] max-w-none rounded-none' : 'w-full max-w-lg rounded-[2rem] max-h-[85vh]'}`}>
                <div className="p-5 border-b border-stone-100 flex justify-between items-center bg-stone-50/80 backdrop-blur sticky top-0 z-10">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => setIsFull(v => !v)} className="p-2 rounded-full bg-stone-100 text-stone-500 hover:text-stone-900 transition-colors" title={isFull ? '还原窗口' : '全屏查看'}>
                            {isFull ? <Minimize2 size={18}/> : <Maximize2 size={18}/>}
                        </button>
                        <button onClick={onClose} className="p-2 rounded-full bg-stone-100 text-stone-400 hover:text-stone-950 transition-colors" title="关闭">
                            <X size={20}/>
                        </button>
                    </div>
                </div>
